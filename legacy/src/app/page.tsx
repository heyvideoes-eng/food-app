...
export default function Home() {
  ...
  useEffect(() => {
    ...
    setIsDemoMode(document.cookie.includes('demo-mode=true'))
  }, [])
      ...
      <main className="min-h-screen pt-24 pb-20 px-6 max-w-7xl mx-auto space-y-12">
        <header className="space-y-4">
          ...
          <h1 className="text-4xl md:text-6xl font-heading font-light tracking-tight text-white">
            Welcome back, <span className="text-primary italic font-medium">{user?.user_metadata?.display_name || (typeof window !== 'undefined' && JSON.parse(localStorage.getItem('fridgemind_user') || '{}').displayName) || 'Guardian'}</span>
          </h1>
        </header>
